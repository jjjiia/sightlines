...
function dataDidLoad(error,sekey,sedata,visiblebgid) {
  ...
  //  drawBarChart(sections[4], sedata,selectedData)    
   // drawChart(total)
    //console.log(sekey)
}
...
function drawBarChart(sectionInfo, sedata,selectedSeData){
    ...
    var margin = {"y":60,"x":300}
    var height = 300
    var width = 500
    var spacer = 2
    var barWidth = height/Object.keys(data).length
    var yScale = d3.scaleLinear()
        .domain([0,1000000])
       .range([2,height])
    
    //
    var svg = d3.select("#chart").append("svg").attr("width",width+margin.x).attr("height",height+margin.y)
    
    svg.append("text").text(sectionInfo.Title).attr("x",spacer).attr("y",margin.y/2)
    ...
    svg.selectAll(".label")
        ...
        .append("text")
        .attr("y",function(d,i){
            return order[d]*barWidth+barWidth/3*2
        })
        .attr("x",function(d){
            return margin.x-spacer
        })
        .attr("text-anchor","end")
        .text(function(d){
            return sectionInfo.Variables[d]
        })
        .attr("transform","translate("+(0)+","+(margin.y)+")")
        .attr("font-size","11px") 
        
    svg.selectAll(".percentLabel")
        .data(Object.keys(percents))
        .enter()
        .append("text")
        .attr("y",function(d,i){
            return order[d]*barWidth+barWidth/3*2
